...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://jack:jack@localhost/jrcleggHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',
    db.Column('search_id', db.Integer, db.ForeignKey('searchterms.id'), primary_key = True),
...
user_collections = db.Table('user_collections',
    db.Column('collection_id', db.Integer, db.ForeignKey('personalgifs.id')),
...
class PersonalGifCollection(db.Model):
    ...
    gifs = db.relationship('Gif', secondary = user_collections, backref = db.backref('personalgifs', lazy = 'dynamic'), lazy = 'dynamic')
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary = search_gifs, backref = db.backref('searchterms', lazy = 'dynamic'), lazy = 'dynamic')
    # TODO 364: Define a __repr__ method for this model class that returns the term string
    def __repr__(self):
        return "{}, Search: {}".format(self.id, self.term)
...
def get_gifs_from_giphy(search_string):
    ...
    requesting = requests.get(baseurl, params = params).text.encode('utf-8')
    jsonDict = json.loads(requesting)
    gifs = []
    for gif in jsonDict['data']:
        gifs.append((gif['title'], gif['embed_url']))
    return gifs

# Provided
def get_gif_by_id(db_session, id):
...
def get_or_create_search_term(db_session, term):
    ...
    search = db_session.query(SearchTerm).filter_by(term = term).first()
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        ...
        for gif in get_gifs_from_giphy(term):
            gif_obj = get_or_create_gif(db_session, gif[0], gif[1])
            search.gifs.append(gif_obj)
...
def get_or_create_collection(db_session, name, current_user, gif_list=[]):
    ...
    collection = PersonalGifCollection.query.filter_by(name = name, userID = current_user.id).first()
    ...
    # However, if no such collection exists, a new PersonalGifCollection instance should be created, and each Gif in the gif_list input should be appended to it (remember, there exists a many to many relationship between Gifs and PersonalGifCollections).
        ...
        collection = PersonalGifCollection(name = name, userID = current_user.id)
        for gif in gif_list:
            gif = get_or_create_gif(db_session, gif.title, gif.embedURL)
...
def validateCollectionsForm(form):
    try:
        if len(form.name.data) != 0 and len(form.gif_picks.data) != 0:
            return True
    except:
        return False
########################
...
def index():
    ...
    if form.validate_on_submit():
        search = get_or_create_search_term(db.session, form.search.data)
...
def search_terms():
    ...
    return render_template('search_terms.html', all_terms = searches)
...
def create_collection():
    ...
    # Use the get_gif_by_id function to create a list of Gif objects.  Then, use the information available to you
    # at this point in the function (e.g. the list of gif objects, the current_user) to invoke the get_or_create_collection
    # function, and redirect to the page that shows a list of all your collections.
    ...
    if validateCollectionsForm(form):
        listIDs = form.gif_picks.data
        gif_obj = [get_gif_by_id(db.session, int(id)) for id in listIDs]
        get_or_create_collection(db.session, form.name.data, current_user, gif_obj)
        return redirect(url_for('collections'))
    else:
        return render_template('create_collection.html', form = form)
    # listIDs = form.gif_picks.data
    # get_or_create_collection(db.session,form.name.data, current_user, listIDs)
    # return redirect(url_for('collections'))
    # try:
    #     listIDs = form.gif_picks.data
    #     get_or_create_collection(db.session, form.name.data, current_user, listIDs)
    #     return redirect(url_for('collections'))
    # except:
    #     flash("Collection not created! Did you enter a name and select a gif?")
    #     return render_template('create_collection.html', form = form)

@app.route('/collections',methods=["GET","POST"])
@login_required
def collections():
    # TODO 364: This view function should render the collections.html template
    #so that only the current user's personal gif collection links will render
    #in that template. Make sure to examine the template so that you send it the correct data!
